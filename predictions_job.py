# one-shot: построить скиллы, скачать расписание и пересчитать прогнозы
import json
...
def fetch_all_matches(c):
    rows = c.execute(text("""
        SELECT date_ev, tour, player_a, player_b, sc_ev, sc_ext_ev FROM matches
    """)).mappings().all()
